function matmul_blocksparse_d(a, b, c, d, bitmap_d,
    ...
    # bm = Layout.load(GLOBAL_D_LAYOUT, bitmap_d, translate_base(bitmap_tile, (M=block_i, N = block_j)))

    bmval = Layout.vloada(Layout.Vec{1, Layout.eltype(GLOBAL_D_LAYOUT)}, pointer(bitmap_d), block_x * BLOCKSPARSE_M + block_y + 1)
    # @CUDA.cuprintln("block $block_x, $block_y: $(block_x * BLOCK_SHAPE.M + block_y + 1), $(bmval[1].value)")
    #
    # return nothing
    ...
    # if warpId == 1 && laneId == 1
    # if thread_x == 1
    #     @CUDA.cuprintln("block $(block_x+1), $(block_y+1) $(block_x * 32 + block_y + 1)")
    # end
